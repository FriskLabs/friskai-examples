 ...
 *
 * `wrapToolset` returns the id-keyed record Mastra's `Agent.tools` expects. The
 * agent is registered on the Mastra instance and returned via `getAgent` so its
 * runs flow through the Frisk exporter.
 */
export function buildMastra(frisk: Frisk): { agent: Agent; memory: Memory } {
  ...
  const agent = new MastraAgent({
    ...
    model: getModel(),
    tools: frisk.wrapToolset(llmTools),
    memory,
  })
