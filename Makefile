...
#   make set-versions -- --example python/basic-langchain-agent --source-env local
...
FORWARD_TARGETS := run example set-versions
...
help:
	...
	@echo '  make set-versions [--] [--example <platform/name>] [--source-env production|local]'
	@echo '      Set frisk SDK versions (all examples if --example unset) and install.'
	@echo '      EXAMPLE=<platform/name> SOURCE_ENV=<env> variables also work.'
...
set-versions:
	./set-versions.sh $(if $(FWD),$(FWD),$(if $(EXAMPLE),--example $(EXAMPLE)) --source-env $(SOURCE_ENV))
